 ...
 */
export default function GroupVisualization({ elements, latticeVectors, showF, fOffset, showGP, gpSeed }) {
  const width = 700;
  const height = 500;
  const svgCx = width / 2;
  ...
  const gpImageUrl = useMemo(() => {
    ...
    const coeffs = drawGPCoefficients(latticeVectors, gpSeed ?? 0, 5, 0.6);
  ...
  }, [showGP, latticeVectors, elements, gpSeed, gpBounds]);
